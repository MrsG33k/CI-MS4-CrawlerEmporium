...
def crawler_support(request):
    ...
    user data, saving tickets to the database, and dispatching confirmation emails.
    """
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ticket_id = ticket.ticket_id_string if ticket.ticket_id_string else "CONFIRMED"

            user_email = form.cleaned_data['email']
            user_name = form.cleaned_data['full_name']
            severity_label = dict(ticket.SEVERITY_CHOICES).get(ticket.severity, ticket.severity)
            ...
            messages.success(
                request, 
                f"Ticket logged under token: {ticket_id}. A confirmation has been sent via email. "
